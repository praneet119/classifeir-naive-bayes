...
#			test_tags= post_tags.split(',')
	test_tags=[]
	for loop in range(len(sys.argv)-2):
		clean_tags=sys.argv[loop+2]
	...
	createfile=open("F:\work\eattreat classes/"+path[max_index]+sys.argv[1]+".txt","a")
	createfile.write(sys.argv[1]+"\t"+"\t"+sys.argv[2])
	for lo in range(len(sys.argv)-3):
		createfile.write(", "+sys.argv[lo+3])
	createfile.close()
